...
class ListDeploymentsInteractor:
    def __init__(self, deployment_repo: IDeploymentRepository):
        self._deployment_repo = deployment_repo

    async def __call__(self, project_id: UUID) -> list[GetDeploymentOutputDTO]:
        deployments = await self._deployment_repo.get_by_project_id(project_id)

        return [
            GetDeploymentOutputDTO(
                id=deployment.id,
